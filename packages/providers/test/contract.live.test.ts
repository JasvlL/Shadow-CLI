 ...
 * skipped unless FLICK_LIVE=1. Run with: FLICK_LIVE=1 npx vitest run contract.live
 */
import { describe, expect, it } from 'vitest';
import { AgyProvider } from '../src/agy.js';
import { ClaudeProvider } from '../src/claude.js';
import type { FlickEvent, Provider } from '../src/types.js';

const live = process.env.FLICK_LIVE === '1';
  ...
  for (const [name, make, model] of cases) {
    it(
      ...
      async () => {
        ...
        const done = events.at(-1) as Extract<FlickEvent, { t: 'done' }>;
